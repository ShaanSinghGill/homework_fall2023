...
class PGAgent(nn.Module):
    ...
    def update(
        ...
        # print(obs.shape, actions.shape, rewards.shape, terminals.shape, q_values.shape)
        ...
        if self.critic is not None:
            ...
            critic_info: dict = baseline_loss
    ...
    def _discounted_return(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for reward in rewards: 
            gammaPows = np.array([self.gamma**i for i in range(len(reward))])
            total = gammaPows * reward
            res.append(np.array([np.sum(total)] * len(reward)))
        return res
    ...
    def _discounted_reward_to_go(self, rewards: Sequence[float]) -> Sequence[float]:
        ...
        for reward in rewards: 
            gammaPows = np.array([self.gamma**i for i in range(len(reward))])
            temp = [reward[t:] * gammaPows[:len(reward)-t] for t in range(len(reward))]
            res.append(np.array([np.sum(r) for r in temp]))
        # print(len(rewards), len(res))
        return res
