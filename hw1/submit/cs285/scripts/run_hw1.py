...
def run_training_loop(params):
    ...
    for itr in range(params['n_iter']):
        ...
        if itr == 0:
            ...
            paths, envsteps_this_batch = utils.sample_trajectories(env, actor, batch_size, params['ep_len'])
            ...
            if params['do_dagger']:
                ...
                for i in range(len(paths)):
                    for j in range(len(paths[i]["observation"])):
                        currob = paths[i]["observation"][j]
                        paths[i]["action"][j] = expert_policy.get_action(currob)
                # paths = TODO
