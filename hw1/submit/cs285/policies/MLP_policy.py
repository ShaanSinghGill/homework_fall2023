...
class MLPPolicySL(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def forward(self, observation: torch.FloatTensor) -> Any:
        ...
        # print(self.mean_net(observation).shape)
        # print("HELLO", distributions.Normal(self.mean_net(observation), torch.exp(self.logstd)).sample(shape))
        # shape = self.mean_net(observation).shape
        # print("HELLO", shape,  distributions.Normal(self.mean_net(observation), torch.exp(self.logstd)).sample().shape)
    ...
    def update(self, observations, actions):
        ...
        pred_actions_dist = self.forward(observations)
        # print(f"shape of pred_actions {pred_actions_dist.shape} and shape of actions {actions.shape}")
        loss = -1 * torch.mean(pred_actions_dist.log_prob(actions))
        loss.backward()
        self.optimizer.step()
        return {
